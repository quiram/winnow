...
- If **more than one tracker** is plausible (e.g. the docs mention both GitHub Issues and JIRA) and the documentation does not disambiguate, **ask the user which one to use — never assume**. In an unattended run, stop and report the ambiguity instead.
